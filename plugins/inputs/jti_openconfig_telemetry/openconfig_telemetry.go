...
type OpenConfigTelemetry struct {
	...
	KeepOriginalTS  bool              `toml:"original_timestamp"`
...
func (m *OpenConfigTelemetry) collectData(ctx context.Context,grpcServer string, grpcClientConn *grpc.ClientConn,acc telegraf.Accumulator) error {
	defer func(){
		m.Log.Infof("Closing gRPC connection with remote %s", grpcServer)
		grpcClientConn.Close()
	} ()
	...
	for _, sensor := range m.sensorsConfig {
		...
		go func(ctx context.Context, sensor sensorConfig) {
			...
			for {
				stream, err := c.TelemetrySubscribe(ctx,&telemetry.SubscriptionRequest{PathList: sensor.pathList})
				if err != nil {
				...
				for {
					...
					if err != nil {
						rpcStatus, _ := status.FromError(err)
						// Canceled mean the parent process has requested a shutdown or a reload, we can finish the goroutine to free resources and allows for grpc connection to reset
						if rpcStatus.Code() == codes.Canceled {
							m.Log.Debugf("Subscription canceled %s: %s", grpcServer, sensor.pathList)
						} else {
					...
					for _, group := range dgroups {
						if m.KeepOriginalTS {
							jtitime := group.data["_timestamp"].(uint64)
							tnow = time.Unix(0,int64(jtitime*1000000))
						}	
						if len(group.tags) == 0 {
...
func (m *OpenConfigTelemetry) Start(acc telegraf.Accumulator) error {
	...
	m.wg.Add(len(m.Servers))
	for _, addr := range m.Servers {
		// Extract device address and port
		grpcServer, _, err := net.SplitHostPort(addr)
		if err != nil {
		...
		go func(address string){
			...
			for ctx.Err() == nil {
				grpcClientConn, err = grpc.DialContext(ctx, address, opts...)
				if err == nil{
					m.Log.Infof("Connection gRPC established with remote %s", address)
					if m.Username != "" && m.Password != "" && m.ClientID != "" {
						...
						if loginErr == nil{
							if loginReply.Result{
								m.Log.Infof("Authentication successful with remote %s", address)
								if err := m.collectData(ctx,grpcServer,grpcClientConn, acc); err != nil && ctx.Err() == nil {
									acc.AddError(err)
								}							
							} else {
								m.Log.Errorf("Failed to authenticate the user for %s", address)
								grpcClientConn.Close()
							}
						} else {
							m.Log.Errorf("Could not initiate login check for %s: %v", address, loginErr)
							grpcClientConn.Close()
						}
					}					
				} else {
					m.Log.Errorf("Failed to connect to %s: %s", address, err.Error())
				}
				select {
				case <-ctx.Done():
				case <-time.After(time.Duration(m.RetryDelay)):
				}
			}
		}(addr)
	}
